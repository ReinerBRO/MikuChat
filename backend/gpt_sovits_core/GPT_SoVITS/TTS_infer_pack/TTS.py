    ...
    def _get_ref_spec(self, ref_audio_path):
        try:
            ...
            if raw_audio.ndim == 1:
                ...
                raw_audio = raw_audio.transpose(0, 1)
        except Exception as e:
            print(f"soundfile load failed in TTS.py: {e}")
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        ...
        with torch.no_grad():
            try:
                ...
                wav16k = torch.FloatTensor(wav16k)
                if wav16k.ndim == 2:
                    wav16k = wav16k.mean(-1) # [Samples, Channels] -> [Samples]
                if sr != 16000:
                    wav16k = resample(wav16k.unsqueeze(0), sr, 16000, self.configs.device).squeeze(0)
                else:
                    wav16k = wav16k.to(self.configs.device)
            except Exception as e:
                print(f"soundfile load failed in _set_prompt_semantic: {e}")
                wav16k, sr = librosa.load(ref_wav_path, sr=16000)
                wav16k = torch.from_numpy(wav16k).to(self.configs.device)
